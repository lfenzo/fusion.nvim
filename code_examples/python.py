...
class Join:
    ...
    def _extract_dist_info(self) -> None:
        ...
        self._rank = dist.get_rank(process=self._process_group)
        self._device = device
    ...
    def __exit__(
        ...
        while not all_procs_joined:
            if i > WARN_THRESHOLD:
                warnings.warn(
                    f"Detected uneven input skew of greater than {WARN_THRESHOLD}. This means that rank "
                    f"{self._rank} has at least {WARN_THRESHOLD} "
                    f"fewer inputs than other currently-active ranks. "
                    "This level of skew could lead to performance degradation during training."
                )
